...
void pubOdometry(const Eigen::Matrix4d& newPose, double& timefullCloud){
  ...
  // DL gets the poses
  tf::Matrix3x3 m = tf::Matrix3x3(laserOdometryTrans.getRotation());
  for(int j = 0; j < 3; j++) outfile<<m[0][j]<< ' ';outfile<<laserOdometryTrans.getOrigin().x()<<' ';
  for(int j = 0; j < 3; j++) outfile<<m[1][j]<< ' ';outfile<<laserOdometryTrans.getOrigin().y()<<' ';
  for(int j = 0; j < 3; j++) outfile<<m[2][j]<< ' ';outfile<<laserOdometryTrans.getOrigin().z()<<'\n';
...
{
  //freopen("/root/soft/livox/data/poses.txt", "w", stdout);
  outfile.open("/root/soft/livox/data/poses.txt");
